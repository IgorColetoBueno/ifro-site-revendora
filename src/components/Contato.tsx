...
export default class Contato extends React.Component<IContatoState> {
          ...
          <section id="formulario-contato">
            <form onSubmit={this.exibirAlerta}>
              <fieldset className="form-group">
                ...
                <input onChange={this.changeNome} required={true} type="text" name="nome" />
              </fieldset>
              <fieldset className="form-group">
                <label htmlFor="email">E-mail</label>
                <input onChange={this.changeEmail} required={true} type="email" name="email" />
              </fieldset>
              <fieldset className="form-group">
                <label htmlFor="descricao">Descrição</label>
                <input onChange={this.changeDescricao} required={true} type="text" name="descricao" />
              </fieldset>
              <fieldset className="form-group">
                <label htmlFor="observacoes">Observações</label>
                <textarea onChange={this.changeObservacoes} required={true} name="observacoes" />
              </fieldset>
  ...
  private changeEmail(event:any){
    this.setState({...this.state, email : event.target.value} as IContatoState);
  }

  private changeDescricao(event:any){
    this.setState({...this.state, descricao : event.target.value} as IContatoState);
  }

  private changeObservacoes(event:any){
    this.setState({...this.state, observacoes : event.target.value} as IContatoState);
  }
